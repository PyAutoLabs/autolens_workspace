...
The scaling relation is the one thing `model.info` cannot show you and the map can. `scaling_galaxies` is drawn as a 
third top-level frame, and inside each member the `einstein_radius` pill is an orange relation pill printing the 
expression itself, `einstein_radius = galaxies.lens.mass.einstein_radius * <ratio>`, with an orange line running 
back to the lens's own `einstein_radius`. Every member of the tier is tied to that one parameter, which is why the 
whole population costs no free parameters of its own.
"""
