...
two different redshifts, forming a double source-plane lens (DSPL). This fits a model where:
...
"""
path_prefix = Path("imaging") / "chaining" / "double_source_plane_lens"
...
In this example, we used prior passing to initialize a model fit to a DSPL. We exploited the fact that 
...
For certain DSPLs, it is possible that the light of the first and second sources are harder to
...
There are currently no pipelines written for DSPLs, albeit one can craft them by learning the API and concepts
from existing template pipelines. We are still figuring out the most effective way to model DSPLs, which is why
pipeline templates are not yet written.
...
at two different redshifts, forming a DSPL. In the final model:
...
The approach used in this pipeline and benefits of using chaining searching to fit DSPLs are
described in the script `notebooks/imaging/features/advanced/double_source_plane_lens/chaining.ipynb`.
...
We use the results of search 4 to create the lens model fitted in search 5, where:
 ...
 - The first source-galaxy's light uses an `Overlay` image-mesh, `RectangularBilinearAdaptDensity` mesh and `Constant` regularization 
 scheme [3 parameters].
 - The second source-galaxy's light uses an `Overlay` image-mesh, `RectangularBilinearAdaptDensity` mesh and `Constant` regularization  
 scheme [3 parameters].
...
In this example, we passed used prior passing to initialize a model fit to a DSPL using 
two `Inversion`'s.

Fitting just the `Inversion` by itself for a DSPL is practically impossible, due to the 
