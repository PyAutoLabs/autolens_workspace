"""
__Log Likelihood Function: DSPL__

This script describes the additional steps required to compute the `log_likelihood` for a double
source-plane lens (DSPL) — a strong lens system with two source galaxies at different redshifts behind the
foreground lens.
...
to DSPL multi-plane ray-tracing.
...
Likelihood") are not repeated here; this script focuses entirely on what changes for a DSPL.
...
For a DSPL, there are TWO source-planes at different redshifts, and the first source galaxy
...
source_1 — this is what makes DSPLs sensitive to cosmology. The factor is computed
...
Load the DSPL dataset. The auto-simulation block mirrors the other example scripts.
"""
...
"""
What `image_2d_from` does internally for our DSPL:
...
print(f"\nLog likelihood of the manual DSPL fit: {fit.log_likelihood}")
...
The DSPL `log_likelihood` differs from the single-plane case in exactly one place: the source
