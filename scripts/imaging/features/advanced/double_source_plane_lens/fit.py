...
A double source-plane lens (DSPL) is a strong lens system where two source galaxies at different redshifts are
lensed by the foreground lens galaxy. They appear as two distinct Einstein rings in the image-plane.

This script illustrates the API for performing a fit to a DSPL via the standard `Tracer`
...
This script focuses on the API specific to a DSPL fit. For background on the underlying single-plane
...
We now compose the three galaxies that form the DSPL:
...
arriving at `source_1`'s plane. This is what makes DSPLs sensitive to angular diameter
...
This script demonstrated the multi-plane ray-tracing and MGE source API for a DSPL, without
...
The key takeaway from this script is that DSPLs are fit with the same `Tracer` + `FitImaging`
