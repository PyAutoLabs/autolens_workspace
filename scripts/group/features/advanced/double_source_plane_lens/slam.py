"""
SLaM (Source, Light and Mass): Group DSPL
=========================================
...
`scripts/imaging/features/advanced/double_source_plane_lens/slam.py`. Each pipeline stage is a plain inline Python
...
def source_pix_1_source_0(
    ...
    source_adapt_image = galaxy_name_image_dict["('galaxies', 'source_0')"].copy()
    source_adapt_image[source_adapt_image > adapt_image_snr_cap] = adapt_image_snr_cap
    galaxy_name_image_dict["('galaxies', 'source_0')"] = source_adapt_image
...
def source_pix_1_source_1(
    ...
    source_adapt_image = galaxy_name_image_dict["('galaxies', 'source_1')"].copy()
    source_adapt_image[source_adapt_image > adapt_image_snr_cap] = adapt_image_snr_cap
    galaxy_name_image_dict["('galaxies', 'source_1')"] = source_adapt_image
...
This pipeline has two source planes: the map of `source_0`, the first source plane, sets the threshold, because
it is the brighter and better constrained of the two.
...
def source_pix_2(
    ...
    source_0_image_raw = galaxy_name_image_dict["('galaxies', 'source_0')"]
    ...
    over_sample_size_pixelization = al.Array2D(
        values=np.where(source_0_image_raw > signal_to_noise_threshold, 4, 2),
        mask=dataset.mask,
    )
...
Load, plot and mask the group DSPL `Imaging` dataset.
"""
