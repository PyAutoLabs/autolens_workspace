...
This script chains two non-linear searches to fit `Imaging` data of a group-scale double source-plane lens (DSPL) —
...
A group-scale DSPL has many more free parameters than either a single-plane group lens OR a single-lens-galaxy
DSPL. A single Nautilus search on the combined model is impractical: the
...
For background on the canonical group lens chaining workflow and the single-lens DSPL chaining
workflow, see:

 - `autolens_workspace/scripts/group/start_here.py` — group `lens_dict` model composition.
 - `autolens_workspace/scripts/imaging/features/advanced/double_source_plane_lens/chaining.py` — single-lens
   DSPL chained search.
"""
...
Load the group DSPL `Imaging` dataset.
"""
...
"""
path_prefix = Path("group") / "chaining" / "double_source_plane_lens"
...
Two chained searches initialised a model for a group-scale DSPL. The key idea is to
...
reconstructions) is shown for the single-lens DSPL in
`autolens_workspace/scripts/imaging/features/advanced/double_source_plane_lens/chaining.py`. The same logic
