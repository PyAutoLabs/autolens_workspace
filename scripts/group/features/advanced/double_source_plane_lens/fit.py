...
This script demonstrates the API for fitting a group-scale DSPL via the standard `Tracer`
...
3-plane ray-tracing structure used in the imaging DSPL example.
...
This script combines the group-scale `lens_dict` API and the DSPL multi-plane API. Read first:

 - `autolens_workspace/scripts/group/fit.py` — the standard group-scale fit.
 - `autolens_workspace/scripts/imaging/features/advanced/double_source_plane_lens/fit.py` — the single-lens DSPL
   fit, including the MGE basis pattern used here.
"""
...
This script demonstrated the API for fitting a group-scale DSPL without invoking a
non-linear search. The two new aspects relative to the imaging DSPL example are:
