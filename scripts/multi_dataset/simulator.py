...
"""
tracer_list = [
    al.Tracer(galaxies=[lens_galaxy, extra_galaxy, source_galaxy], fields=[field])
...
example (`multi_dataset/modeling.py`) can load each one and apply noise scaling. The mask is built per dataset because the
...
"""
The dataset can be viewed in the folder `autolens_workspace/dataset/multi_dataset/imaging/lens_sersic`.
"""
