...
- **JAX:** JIT-ing `LensCalc` methods and other library calls yourself — see `scripts/guides/using_jax.py`.
...
`LensCalc` calculations are JAX-accelerated automatically inside a model-fit. The "JIT-it-yourself" advanced
path — wrapping `LensCalc` methods or `tracer.image_2d_from` in your own `@jax.jit` — is covered by the
executable recipes in `scripts/guides/using_jax.py`.
