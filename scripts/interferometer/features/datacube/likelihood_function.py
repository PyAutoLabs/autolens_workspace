...
pixelized source reconstruction (specifically a `RectangularBilinearAdaptDensity` mesh and `Constant` regularization
scheme).
...
This example uses a `RectangularBilinearAdaptDensity` mesh + `Constant` regularization — the same combination used by
...
`RectangularBilinearAdaptDensity`; the linear algebra is identical and the construction code is the same. The single
behaviour difference is that `RectangularBilinearAdaptDensity` lets the mesh's pixel density adapt to the source-plane
...
- **Simplifications:** `RectangularBilinearAdaptDensity` mesh, `Constant` regularization.
...
`RectangularBilinearAdaptDensity` substituted for `RectangularUniform`. The classes share the same construction
machinery — `RectangularUniform` is a subclass of `RectangularBilinearAdaptDensity` — so all of the mesh-grid and
...
parameters using a non-linear search. For the user-facing modeling story see:
 ...
 - `modeling.py` — `RectangularBilinearAdaptDensity` pixelization fit with `af.Nautilus`, the canonical entry point.
