...
pixelization examples use a `RectangularRTUAdaptDensity` mesh, which adapts the size of source pixels to the
...
concepts used in this script:
 ...
 - `imaging/features/linear_light_profiles/likelihood_function.py` the likelihood function for a linear light profile, which
...
The `RectangularRTUAdaptDensity` mesh pretty much works exactly the same, its just that a calculation (which we don't
describe here) works out how to make a grid of rectangular pixels that adapt to the source-plane density and thus
vary in size. 

There is also a `RectangularRTUAdaptImage` mesh which uses the image of the lensed source to adapt
...
are described in additional notebooks found in this package. In brief, these describe:
 ...
 paired fractionally with each `RectangularRTUAdaptDensity` pixel.

 - **Source-plane Interpolation**: Using bilinear interpolation on the `RectangularRTUAdaptDensity` pixelization to pair 
 each image (sub-)pixel to multiple `RectangularRTUAdaptDensity` pixels with interpolation weights.
