The `double_source_plane_lens` folder contains example scripts for analysing **group-scale** double-source-plane
...
- `likelihood_function`: Step-by-step description of the additional likelihood-function steps specific to a
  group-scale DSPL (multi-plane deflection chain across multiple main lens galaxies).

# Background

For background on the multi-plane physics, the cosmological sensitivity (`beta_01` distance ratio), and the
imaging-data API, see the single-lens-galaxy DSPL example at
